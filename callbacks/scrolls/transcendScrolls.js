...
module.exports = [["scrolls.transcend_scroll", function (session, callback) {
    deleteMessage(callback.message.chat.id, session.messages, callback.message.message_id);

        let buttons = [];

    axios.get(`http://localhost:8080/scroll?type=${types.transcendScroll["transcend_scroll"]}&summonsCount=${session.scrolls.counter ? session.scrolls.counter : 1}`)
        .then(r => {
