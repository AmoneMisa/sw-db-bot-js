...
module.exports = [[/^scrolls\.mystic_scroll\./, function (session, callback) {
    deleteMessage(callback.message.chat.id, session.messages, callback.message.message_id);
    const [, type] = callback.data.match(/^scrolls\.mystic_scroll\.(.*)$/);

    let buttons = [];

    axios.get(`http://localhost:8080/scroll?type=${types.mysticScroll[type]}&summonsCount=${session.scrolls.counter ? session.scrolls.counter : 1}`)
        .then(r => {
