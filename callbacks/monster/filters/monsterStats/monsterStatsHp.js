...
module.exports = [["monsters.filter.type.stats.hp", function (session, callback) {
    ...
    sendMessage(session, callback.message.chat.id, `${dictionary[session.language.text].monsters.stats.hp.filter}`, {
        reply_markup: {
            inline_keyboard: [[{
    ...
    deleteMessage(callback.message.chat.id, session.messages, callback.message.message_id);
    sendMessage(session, callback.message.chat.id, `${dictionary[session.language.text].monsters.stats.hp.gte}`, {
        reply_markup: {
            force_reply: true
        }
    }).then((msg) => {
    ...
    deleteMessage(callback.message.chat.id, session.messages, callback.message.message_id);
    sendMessage(session, callback.message.chat.id, `${dictionary[session.language.text].monsters.stats.hp.lte}`, {
        reply_markup: {
            force_reply: true
        }
    }).then((msg) => {
